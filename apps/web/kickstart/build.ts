...
function buildAgentsMarkdown(
  ...
  const markdownCapabilityLines = [
    ...
    '- Interaction is live in the app: tooltips, hover, pan/zoom, brushing and cross-filtering all work. Charts also expose an export menu (PNG/SVG) and a spec viewer.',
    ...
    '- Safety limits, which are not feature restrictions: a spec may not exceed 8 MB or 200k inline rows (counting `data.values`, named `datasets`, GeoJSON features and generated `sequence` rows), and `javascript:` links are removed.',
    '- The row budget counts inline data only, and so does the renderer\'s choice of drawing mode. A very large `data.url` file therefore stays within the limits but can still make a chart slow — inline the data, or split the figure, when the row count is in the millions.',
    '- Cost is driven by the number of views as well as the number of rows. A wide `repeat` or `facet` product multiplies work even on small data: a 40x40 repeat is 1600 plots and will take seconds. PDF export cancels a chart that overruns; in the app an extreme spec can make the tab unresponsive, so keep the view count reasonable and prefer several figures over one enormous grid.',
