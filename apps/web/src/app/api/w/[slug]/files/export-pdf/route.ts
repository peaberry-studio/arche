  ...
  async (request: NextRequest, { slug }) => {
    ...
    try {
      ...
      if (abortContext.signal.aborted) {
        throw getWorkspaceExportAbortReason(abortContext.signal)
      }
      ...
      const html = await markdownToPdfHtml(
        bundleResult.bundle,
        { logoBase64: LOGO_BASE64, signal: abortContext.signal },
      )
      const pdf = await pagedHtmlToPdf(html, abortContext.signal)
      ...
      if (
        error instanceof PdfExportTimeoutError ||
        error instanceof WorkspaceExportTimeoutError
      ) {
