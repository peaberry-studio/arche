...
describe('EditFlowPage', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    mocks.getDesktopFlowsHref.mockReturnValue('/w/local?flows=edit&flowId=flow-1')
  })
  ...
  it('redirects desktop flow editing into the workspace flow dialog', async () => {
    mocks.isDesktop.mockReturnValue(true)
    mocks.getCurrentDesktopVault.mockReturnValue({ vaultId: 'vault-1', vaultName: 'Vault', vaultPath: '/tmp/vault' })
    mocks.getRuntimeCapabilities.mockReturnValue({ flows: true })
    const Page = (await import('../page')).default

    await expect(Page({ params: Promise.resolve({ id: 'flow-1', slug: 'alice' }) })).rejects.toThrow('REDIRECT:/w/local?flows=edit&flowId=flow-1')
    expect(mocks.getDesktopFlowsHref).toHaveBeenCalledWith('local', 'edit', 'flow-1')
  })
})
