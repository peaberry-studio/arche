...
type DocumentPathResult =
  | { ok: true; path: string }
  | { ok: false; response: Response }

type DocumentBundleResult =
...
export async function parseWorkspaceDocumentPath(request: Request): Promise<DocumentPathResult> {
  let body: unknown
  try {
  ...
  return isValidWorkspacePath(normalizedPath, { extension: ".md" })
    ? { ok: true, path: normalizedPath }
    : { ok: false, response: jsonResponse(400, { error: "invalid_path" }) }
}

export async function gatherWorkspaceDocumentBundle(
  ...
  const listOptions = { markdownOnly: true, maxEntries: MAX_LISTED_MARKDOWN_ENTRIES }
