...
export async function handleAhrefsMcpRequest(
  ...
  const parsedConfig = parseAhrefsConnectorConfig(decryptedConfig)
  if (!parsedConfig.ok) {
    return jsonRpcError(
      null,
      -32000,
      parsedConfig.message ?? `Invalid Ahrefs connector config: ${parsedConfig.missing?.join(', ')}`,
      500
    )
  }
  ...
  switch (body.method) {
    case 'initialize':
      return jsonRpcResult(id, {
        protocolVersion: AHREFS_MCP_PROTOCOL_VERSION,
        capabilities: {
          tools: {
            listChanged: false,
          },
    ...
    case 'tools/call': {
      ...
      const result = await executeAhrefsMcpTool(parsedConfig.value, toolName, params?.arguments)
