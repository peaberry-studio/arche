...
export async function handleUmamiMcpRequest(
  ...
  const parsedConfig = parseUmamiConnectorConfig(decryptedConfig)
  if (!parsedConfig.ok) {
    return jsonRpcError(
      null,
      -32000,
      parsedConfig.message ?? `Invalid Umami connector config: ${parsedConfig.missing?.join(', ')}`,
      500
    )
  }
  ...
  switch (body.method) {
    ...
    case 'tools/call': {
      ...
      const result = await executeUmamiMcpTool(parsedConfig.value, toolName, params?.arguments)
