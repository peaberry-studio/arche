...
export async function handleMetaAdsMcpRequest(
  ...
  if (!parsedConfig.ok) {
    return jsonRpcError(
      null,
      -32000,
      parsedConfig.message ?? `Invalid Meta Ads connector config: ${parsedConfig.missing?.join(', ')}`,
      500
    )
  }
  ...
  switch (body.method) {
    ...
    case 'tools/list':
      return jsonRpcResult(id, {
        tools: getMetaAdsMcpTools(parsedConfig.value),
      })
    ...
    case 'tools/call': {
      ...
      const result = await executeMetaAdsMcpTool(decryptedConfig, toolName, params?.arguments)
