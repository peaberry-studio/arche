...
describe('kbGithubRemoteService', () => {
  ...
  it('findIntegration returns record with parsed state', async () => {
    mockPrismaFindUnique.mockResolvedValue(
      makeRow('enc:{"repoUrl":"https://github.com/owner/repo.git","pat":"ghp_abc"}', {
  ...
  it('saveIntegrationConfig preserves existing PAT when not provided', async () => {
    mockPrismaFindUnique.mockResolvedValue(
      makeRow('enc:{"repoUrl":"https://github.com/old/repo.git","pat":"ghp_existing"}'),
    )

    const { saveIntegrationConfig } = await import('../kb-github-remote')
    await saveIntegrationConfig({ repoUrl: 'https://github.com/new/repo.git' })
  ...
  it('saveIntegrationConfig stores new PAT when provided', async () => {
    mockPrismaFindUnique.mockResolvedValue(
      makeRow('enc:{"repoUrl":"https://github.com/old/repo.git","pat":"ghp_old"}'),
    )

    const { saveIntegrationConfig } = await import('../kb-github-remote')
    await saveIntegrationConfig({ repoUrl: 'https://github.com/new/repo.git', pat: 'ghp_new' })
  ...
  it('isConfigured returns true when both repoUrl and pat exist', async () => {
    mockPrismaFindUnique.mockResolvedValue(
      makeRow('enc:{"repoUrl":"https://github.com/owner/repo.git","pat":"ghp_abc"}'),
    )
  ...
  it('isConfigured returns false when pat is missing', async () => {
    mockPrismaFindUnique.mockResolvedValue(
      makeRow('enc:{"repoUrl":"https://github.com/owner/repo.git"}'),
    )
