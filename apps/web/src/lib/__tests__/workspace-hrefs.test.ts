...
describe("getWorkspaceHref", () => {
  it("builds workspace paths with normalized query parameters", () => {
    ...
    expect(getWorkspaceHref("alice", { mode: "explore" })).toBe("/w/alice?mode=explore");
    ...
    expect(getWorkspaceHref("alice", { path: "Notes/Brief.md" })).toBe("/w/alice?mode=explore&path=Notes%2FBrief.md");
  });
});
