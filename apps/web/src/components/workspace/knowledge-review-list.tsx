...
const ERROR_LABELS: Record<string, string> = {
  ...
  not_found: 'Knowledge Review change not found.',
  not_open: 'This Knowledge Review change is no longer open.',
  not_rebaseable: 'This Knowledge Review change cannot be rebased.',
  ...
  knowledge_review_load_failed: 'Could not load Knowledge Review changes.',
...
export function KnowledgeReviewList({ onApplied, refreshKey = 0, slug }: KnowledgeReviewListProps) {
  ...
  return (
    <section className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        <h3 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Knowledge changes</h3>
      ...
      {openChanges.length === 0 ? <p className="text-xs text-muted-foreground">{isLoading ? 'Loading Knowledge Review…' : 'No knowledge changes awaiting review.'}</p> : null}
    </section>
  )
}
