...
describe('kickstart manifests', () => {
  ...
  it('marketing template exposes curated prompt overrides', () => {
    const marketing = getKickstartTemplateById('marketing-studio')
    expect(marketing).not.toBeNull()
    if (!marketing) return

    expect(marketing.promptOverrides['performance-marketing']).toContain('You are')
    expect(marketing.promptOverrides['performance-marketing']).toContain('Hypothesis')
    expect(marketing.promptOverrides.copywriter).toContain('Awareness Levels')
    expect(marketing.promptOverrides['ads-scripts']).toContain('Hook')
    expect(marketing.promptOverrides.seo).toContain('E-E-A-T')

    expect(marketing.promptOverrides['performance-marketing']).not.toContain('Eres')
    expect(marketing.promptOverrides.copywriter).not.toContain('Objetivo')
    expect(marketing.promptOverrides['ads-scripts']).not.toContain('Reglas')
    expect(marketing.promptOverrides.seo).not.toContain('Proceso obligatorio')
  })
  ...
  it('template summaries include prompt overrides for client preview', () => {
    ...
    expect(marketingSummary!.promptOverrides.copywriter).toContain('Awareness Levels')
  })
})
